...
in
{
  options.dotfiles.hyprlandLua = lib.mkOption {
    type = lib.types.raw;
    internal = true;
  };
  config.dotfiles.hyprlandLua = # Helper functions for Hyprland Lua config generation via Home Manager.
    lib:
    let
    ...
    rec {
      ...
      workspaceBinds =
        ...
        in
        lib.concatMap (w: mkPair w.key w.id) [
          ws.study
          ws.browser
          ws.terminal
          ws.editors
